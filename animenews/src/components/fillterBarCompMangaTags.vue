<template>
  <section class="app flex mb-10 flex-col gap-3">
    ...
    <div class="flex items-center justify-between font-raleway font-medium">
      ...
      <router-link :to="generateRoute">
        <button
          class="px-1 py-1 w-40 text-zinc-950 hover:text-zinc-50 dark:text-zinc-50 dark:hover:text-zinc-950 border-zinc-950 hover:border-zinc-50 dark:hover:border-zinc-950 border-2 dark:border-zinc-50 dark:bg-zinc-950 dark:hover:bg-zinc-50 bg-zinc-50 hover:bg-zinc-950 hover:font-bold transition-all duration-200 ease-linear"
        >
          search
        </button>
      </router-link>
  ...
  computed: {
    generateRoute() {
      ...
      if (this.seasonQueryValues !== '') {
        query.country = this.seasonQueryValues
      }
